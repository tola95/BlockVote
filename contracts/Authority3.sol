...
contract Authority3  {
    ...
    function Authority3(address adminAddress) {
    ...
    function sendShare(address authorityAd, address authority2Ad) {
        Authority(authorityAd).receiveShare(secret, shareIndex);
    ...
    function calculateRecombinant(Share[] shares, Share share, int P) private returns (int) {
        int recombs = 1;
        for (uint8 i=0; i<shares.length; i++) {
            if (share.secret != shares[i].secret) {
